...
static void lvgl_port_task(void *arg)
{
    ...
    while (1) {
        if (LVGL_lock(-1)) { // Try to lock the LVGL mutex
            task_delay_ms = lv_timer_handler(); // Handle LVGL timer events
            LVGL_unlock(); // Unlock the mutex
        }
...
esp_err_t LVGL_Init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle)
{
...
bool LVGL_notify_rgb_vsync(void)
{
