...
static esp_err_t i2c_master_init(void)
{
    ...
    // 1) Tenter de récupérer le bus déjà créé par le BSP (port 0 par ex.)
    if (i2c_master_get_bus_handle(I2C_MASTER_NUM, &s_i2c_bus) == ESP_OK && s_i2c_bus) {
        ESP_LOGI("I2C", "Reusing BSP I2C bus (port %d)", I2C_MASTER_NUM);
        return ESP_OK;
    }

    // 2) Sinon, on le crée nous-mêmes (toujours NG)
